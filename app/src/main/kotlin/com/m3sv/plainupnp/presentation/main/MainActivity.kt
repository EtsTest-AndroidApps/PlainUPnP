...
class MainActivity : ComponentActivity() {
        ...
        setContent {
            ...
            AppTheme(currentTheme.isDarkTheme()) {
                BottomSheetScaffold(
                    ...
                    Box {
                        when {
                            configuration.isInLandscape -> {
                                window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN)
                                Landscape(
                                    upnpState = viewState.playbackState,
    ...
    private fun Portrait(
        ...
        Column {
            ...
            loadingIndicator()
            Row {
                sortingRow(Modifier.padding(start = 8.dp))
                Spacer(modifier = Modifier.weight(1f))
            }
    ...
    @Composable
    private fun Controls(playbackState: PlaybackState, modifier: Modifier = Modifier) {
        when (playbackState) {
            is PlaybackState.Active -> Column(modifier = modifier) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Text(playbackState.title)
                }

                Slider(
                    value = playbackState.elapsedPercent.toFloat() / 100,
                    onValueChange = {
                        viewModel.moveTo((it * 100).toInt())
                    })

                Row(horizontalArrangement = Arrangement.Center) {
                    Text(playbackState.position)
                    Text("/")
                    Text(playbackState.duration)
                }
                ...
                Row(
                    ...
                    IconButton(onClick = {
                        val button = if (playbackState.state == TransportState.PAUSED_PLAYBACK)
                            PlayerButton.PLAY
                        else
                            PlayerButton.PAUSE
            ...
            PlaybackState.Empty -> {
                Column(
                    ...
                    Image(
                        painterResource(R.drawable.ic_duck),
                        contentDescription = stringResource(R.string.nothing_to_play)
                    )
                    Text(stringResource(R.string.nothing_to_play), modifier = Modifier.padding(8.dp))
                }
            }
        }
    }
    ...
    private val PlaybackState.Active.icon: Int
    ...
    private inline val Configuration.isInLandscape
        get() = orientation == Configuration.ORIENTATION_LANDSCAPE

    private inline val Configuration.isInPortrait
